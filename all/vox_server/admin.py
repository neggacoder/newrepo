...
WRITABLE_DIRS = [
    ...
    # Два разных каталога шаблонов (после объединения серверов их пришлось
    # развести по именам): ocr_templates/ — распознавание документов,
    # templates/ — последовательности шагов из режима «Обучение».
    os.path.realpath(os.path.join(HERE, "ocr_templates")),
...
async def get_ai_settings(_auth: bool = Depends(require_auth)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Модуль vox_server.py недоступен: {e}")
    cfg = server.CONFIG
    return {
