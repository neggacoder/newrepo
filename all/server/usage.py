...
ПРИМЕЧАНИЕ: Whisper (речь→текст) работает локально, токенов не возвращает и
тарифицируется по минутам аудио — сюда не попадает.
...
def record_usage(provider, model, usage, prices=None, when=None, endpoint=None):
    ...
    `endpoint` — откуда пришёл вызов: 'command' | 'ocr-template'.
